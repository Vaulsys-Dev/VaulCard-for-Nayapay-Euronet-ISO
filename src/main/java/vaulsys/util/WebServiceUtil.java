...
//        }
    	...
    	//Added by Moiz for  VP-NAP-202403211 ==> CMF - Visa DE62 Enablement
        if(Util.hasText(wsmodel.getCustomPaymentService()))
        {
            logger.info("CPS [" + wsmodel.getCustomPaymentService() + "]");
        }
		//=========================================================================================
        ...
        if (Util.hasText(wsmodel.getIsinternational())) {
            logger.info("Isinternational [" + wsmodel.getIsinternational() + "]");
        }

        if (Util.hasText(wsmodel.getAcqreferenceno())) {
            logger.info("Acqreferenceno [" + wsmodel.getAcqreferenceno() + "]");
        }

        if (Util.hasText(wsmodel.getCentralprocessingdate())) {
            logger.info("Centralprocessingdate [" + wsmodel.getCentralprocessingdate() + "]");
        }

        if (Util.hasText(wsmodel.getSchemetransactionid())) {
            logger.info("Schemetransactionid [" + wsmodel.getSchemetransactionid() + "]");
        }

        if (Util.hasText(wsmodel.getSourceamount())) {
            logger.info("Sourceamount [" + wsmodel.getSourceamount() + "]");
        }

        if (Util.hasText(wsmodel.getSourcecurrency())) {
            logger.info("Sourcecurrency [" + wsmodel.getSourcecurrency() + "]");
        }
        // ==========================================================
        
		//m.rehman: 09-01-2023, VP-NAP-202301071 - DCC settlement transaction charge and WHT (Priority Ticket)
        if (Util.hasText(wsmodel.getForexbufferamount())) {
            logger.info("ForexBufferAmount [" + wsmodel.getForexbufferamount() + "]");
        }

        //m.rehman: 06-02-2023 - VP-NAP-202302021 / VC-NAP-202302021 - Visa fee and additional Nayapay charges on Visa txn + DCC settlement txn charge and WHT
        if (wsmodel.getDccflag() != null) {
            logger.info("DccFlag [" + wsmodel.getDccflag() + "]");
        }

        if (Util.hasText(wsmodel.getIntltxnfee())) {
            logger.info("IntlTxnFee [" + wsmodel.getIntltxnfee() + "]");
        }

        if (Util.hasText(wsmodel.getIntltxnfeeinusd())) {
            logger.info("IntlTxnFeeInUSD [" + wsmodel.getIntltxnfeeinusd() + "]");
        }

        if (Util.hasText(wsmodel.getIntltxnfeetax())) {
            logger.info("IntlTxnFeeTax [" + wsmodel.getIntltxnfeetax() + "]");
        }

        if (Util.hasText(wsmodel.getNayapayinterbankusdrate())) {
            logger.info("NayapayInterbankUSDRate [" + wsmodel.getNayapayinterbankusdrate() + "]");
        }

        if (Util.hasText(wsmodel.getSettbankspread())) {
            logger.info("SettBankSpread [" + wsmodel.getSettbankspread() + "]");
        }

        if (Util.hasText(wsmodel.getNayapayrevenuebuffer())) {
            logger.info("NayapayRevenueBuffer [" + wsmodel.getNayapayrevenuebuffer() + "]");
        }
        // Asim Shahzad, Date : 11th May 2023, Tracking ID : VP-NAP-202302021 / VC-NAP-202302021
        if (Util.hasText(wsmodel.getInternationaltxnfailedfee())) {
            logger.info("InternationalTxnFailedFee [" + wsmodel.getInternationaltxnfailedfee() + "]");
        }
        // =====================================================================================
        ...
        if (Util.hasText(wsmodel.getInternationaltxnfeepercent())) {
            logger.info("IntlTxnFeePercent [" + wsmodel.getInternationaltxnfeepercent() + "]");
        }
        if (Util.hasText(wsmodel.getInternationaltxnfeepercenttax())) {
            logger.info("IntlTxnFeePercentTax [" + wsmodel.getInternationaltxnfeepercenttax() + "]");
        }

        if (Util.hasText(wsmodel.getInternationaltxnfailedfeepercent())) {
            logger.info("IntlTxnFailedFeePercent [" + wsmodel.getInternationaltxnfailedfeepercent() + "]");
        }
        if (Util.hasText(wsmodel.getInternationaltxnfailedfeepercenttax())) {
            logger.info("IntlTxnFailedFeePercentTax [" + wsmodel.getInternationaltxnfailedfeepercenttax() + "]");
        }
        // =====================================================================================

        //Added By Waleed
        if(wsmodel.getIsmerchanttransaction() != null) {
            logger.info("IsMerchantTransaction [" + wsmodel.getIsmerchanttransaction() + "]");
        }
        ...
        if(wsmodel.getCardid() != null) {
            logger.info("CardId [" + wsmodel.getCardid() + "]");
        }
