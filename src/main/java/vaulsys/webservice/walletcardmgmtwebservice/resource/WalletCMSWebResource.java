...
public class WalletCMSWebResource {
	...
	//m.rehman: 17-12-2021, VP-NAP-202112102/VC-NAP-202112171 - Credit Refunds issues, updating servicename to prevent from showing in list
	//and adding Credit refund case separately
	@Path("/creditrefund")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})  //add MediaType.APPLICATION_XML if you want XML as well (don't forget @XmlRootElement)
	public WalletCMSWsEntity CreditRefund(WalletCMSWsEntity wsobj) throws Exception{

		try {
			logger.info("WebResource CreditRefund Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setServicename("CreditRefund");
			//m.rehman: for validating incoming ip address
			wsobj.setIncomingip(request.getRemoteAddr());
			//create thread here
			//ExecutorService executor = MessageManager.threadPool;
		...
		{
			e.printStackTrace();
			logger.error("Exception caught while executing CreditRefund call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending CreditRefund reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		//m.rehman: 15-11-2021, Nayapay Optimization
		finally {
	...
	@Path("/transactionnotfoundreversal")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public WalletCMSWsEntity TransactionNotFoundReversal(WalletCMSWsEntity wsobj) throws Exception {
		try {
			logger.info("WebResource TransactionNotFoundReversal Request Received");
			if(wsobj == null)
			{
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
			...
			wsobj.setServicename("TransactionNotFoundReversal");
			//m.rehman: for validating incoming ip address
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
		...
		{
			logger.error("Exception caught while executing TransactionNotFoundReversal call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending TransactionNotFoundReversal reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		//m.rehman: 15-11-2021, Nayapay Optimization
		finally {
	...
	@Path("/visarefundtransaction")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})  //add MediaType.APPLICATION_XML if you want XML as well (don't forget @XmlRootElement)
	public WalletCMSWsEntity VISARefundTransaction(WalletCMSWsEntity wsobj) throws Exception {

		try {
			logger.info("WebResource VISARefundTransaction Request Received");
			if (wsobj == null) {
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
				wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			...
			wsobj.setServicename("VISARefundTransaction");
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("Exception caught while executing VISARefundTransaction call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending VISARefundTransaction reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		//m.rehman: 15-11-2021, Nayapay Optimization
		finally {
	...
	@Path("/chargecustomertransaction")
	@POST
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})  //add MediaType.APPLICATION_XML if you want XML as well (don't forget @XmlRootElement)
	public WalletCMSWsEntity ChargeCustomerTransaction(WalletCMSWsEntity wsobj) throws Exception {

		try {
			logger.info("WebResource ChargeCustomerTransaction Request Received");
			if (wsobj == null) {
				logger.error("No Data Received from request! replying..");
				wsobj = new WalletCMSWsEntity();
				wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			...
			wsobj.setServicename("ChargeCustomerTransaction");
			wsobj.setIncomingip(request.getRemoteAddr());
			SimpleMainWSProcess process = new SimpleMainWSProcess(WalletCMSWSServer.id++,wsobj);
			return process.processTransaction();
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("Exception caught while executing ChargeCustomerTransaction call");
			logger.error(WebServiceUtil.getStrException(e));
			wsobj.setRespcode(ISOResponseCodes.NP_AUTHENTICATION_FAILED);
			logger.error("Sending ChargeCustomerTransaction reply [" + wsobj.getRespcode() + "]");
			return wsobj;
		}
		finally {
